...
const ProjectStatus = require ('./project-status.js');
...
module.exports = class Sync {
  ...
  complete(cb){
    ...
    try{
      _this.utils.rmdir(this.project_path + "/" + _this.constants.compressed_files_folder,function(){
        lcb()
      })
    }catch(e){
  ...
  writeFilesRecursive(files,cb,depth){
    ...
    var write_continue = function(files,cb,depth){
      if(files.length == 0){
        ...
        var check_hash = size < _this.constants.hash_check_max_size
        if(type == "f"){
          try{
