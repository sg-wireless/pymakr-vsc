...
test("Can find devices", async () => {
  assert(pymakr.devicesStore.get().length >= 1);
  ...
  test("device", async () => {
    ...
    test("can create REPL", async () => {
      ...
      test("can run large script", async () => {
        pymakr.commands.runScript(
          readFileSync(__dirname + "/file-management/_sample/folder/large-file.py", "utf8"),
          device
        );
      ...
      test("can configure chunking", async () => {
        ...
        assert(
          withChunk.time > withoutChunk.time * 1.5,
          `Chunking should be slower ${withChunk.time} > (${withoutChunk.time} * 1.5)`
        );
      });
